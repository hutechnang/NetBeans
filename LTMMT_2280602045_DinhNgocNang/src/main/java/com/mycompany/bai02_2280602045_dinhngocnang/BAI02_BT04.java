 ...
 */
public class BAI02_BT04 {
    public static void main(String[] args) {
        String fileName = "orders.dat";
        try {
            System.out.println("=== TASK 1: GHI DU LIEU VAO FILE orders.dat (SU DUNG LUONG BYTE) ===");
            writeOrdersToFile(fileName);
            System.out.println("Da ghi du lieu thanh cong vao file " + fileName + "\n");
            
            System.out.println("=== TASK 2: DOC DU LIEU VA TINH TONG TIEN DON HANG PENCIL (SU DUNG LUONG BYTE) ===");
    ...
    public static void writeOrdersToFile(String fileName) throws IOException {
        try (FileOutputStream fos = new FileOutputStream(fileName)) {
            fos.write("Ngay thuc hien,Trang thai,San pham,So luong,Don gia,Tong tien\n".getBytes());
            String[] orders = {
            ...
            for (String order : orders) fos.write(order.getBytes());
    ...
    public static double calculateTotalPencilOrders(String fileName) throws IOException {
        try (FileInputStream fis = new FileInputStream(fileName)) {
            ...
            int byteData;
            while ((byteData = fis.read()) != -1) content.append((char) byteData);
